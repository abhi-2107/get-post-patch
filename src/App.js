...
function App() {
  ...
  // abhi to parameters de diye hai but I think REDUX is the correct way ? kya khta bhai
  ...
  let pageNumber = [...Array(posts.length / 10)].map((_, index) => (<span className= {(page === index+1) ? "selected__page" : ""} onClick={() => pageSelected(index + 1)} key={index}>{index + 1}</span>))
  ...
  return (
    <div>
      ...
      <div className="pagination">
        <span  onClick={() => pageSelected(page -1)}><b>PREV</b></span>
        {pageNumber}
        <span onClick={() => pageSelected(page + 1)}><b>NEXT</b></span>
      </div>
  ...
  function AddPostForm() {
    // in tino variables ko postreq function mai use kar na hai
